...
class Task2():
    ...
    def next_action(self, state):
        ...
        if state[1] > safe_y_n and state[1] < safe_y_p:
            ...
            (state[0]>0 and state[1]>0 and (abs(direct[0][0]-state[0]) or direct[0][1] > state[1]) > th) or
            (state[0]>0 and state[1]<0 and (abs(direct[1][0]-state[0]) or direct[1][1] < state[1]) > th) or
            (state[0]<0 and state[1]>0 and (abs(direct[2][0]-state[0]) or direct[2][1] > state[1]) > th) or
            (state[0]<0 and state[1]<0 and (abs(direct[2][0]-state[0]) or direct[3][1] < state[1]) > th)
        ):
            if state[1] > 0:
